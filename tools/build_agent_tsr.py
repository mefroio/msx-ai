...
class AgentTsrOutputs:
    ...
    driver_16c550_115200_path: pathlib.Path
    driver_unapi_path: pathlib.Path
    size: int
    transport_file_offset: int
    divisor_file_offset: int
...
def _check_linked_images(images: Sequence[LinkedImage]) -> dict[str, int]:
    ...
    for name in (
            "active_transport_id", "active_uart16c550_divisor",
            "resident_keyi_hook",
    ...
    divisor_offset = offsets["active_uart16c550_divisor"]
    if primary.data[divisor_offset] != DIVISOR_57600:
        raise AgentTsrBuildError(
            "active_uart16c550_divisor does not contain divisor 2")
    return offsets
...
def _metadata(size: int, transport_file_offset: int,
              divisor_file_offset: int) -> bytes:
    return (
        ...
        "MSXAI_TSR_16C550_DIVISOR_OFFSET: equ "
        f"{_assembly_number(divisor_file_offset)}\n"
    ).encode("ascii")
...
def build_agent_tsr(
        ...
        driver_16c550_115200_output: pathlib.Path | None = None,
    ...
    divisor_file_offset = (
        HEADER_SIZE + relocation_table_length
        + offsets["active_uart16c550_divisor"])
    if result.data[divisor_file_offset] != DIVISOR_57600:
        raise AgentTsrBuildError(
            "computed divisor patch offset does not address divisor 2")
    ...
    if driver_16c550_115200_output is None:
        driver_16c550_115200_output = output.with_name("MCP115K.TSR")
    ...
    if len({output.resolve(), driver_8251_output.resolve(),
            ...
            driver_16c550_115200_output.resolve(),
            driver_unapi_output.resolve()}) != 5:
        raise AgentTsrBuildError(
    ...
    driver_16c550_115200 = bytearray(driver_16c550)
    driver_16c550_115200[divisor_file_offset] = DIVISOR_115200
    ...
    baud_differing = tuple(
        index for index, values in enumerate(zip(
            driver_16c550, driver_16c550_115200, strict=True))
        if len(set(values)) != 1)
    if baud_differing != (divisor_file_offset,):
        raise AgentTsrBuildError(
            "16C550 baud TSRs must differ only at the divisor byte")

    metadata = _metadata(
        len(result.data), transport_file_offset, divisor_file_offset)
    ...
    _atomic_write(
        driver_16c550_115200_output, bytes(driver_16c550_115200))
    ...
    return AgentTsrOutputs(
        ...
        driver_16c550_115200_output, driver_unapi_output, len(result.data),
        transport_file_offset, divisor_file_offset, result.relocation_offsets)
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--16c550-115200-output", dest="driver_16c550_115200_output",
        type=pathlib.Path,
        default=REPOSITORY / "work" / "agent" / "MCP115K.TSR")
    parser.add_argument(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        outputs = build_agent_tsr(
            ...
            args.driver_16c550_output, args.driver_16c550_115200_output,
            args.driver_unapi_output,
    ...
    print(
        f"wrote {outputs.driver_16c550_115200_path}: "
        "transport 16C550 at 115200")
