...
class Session:
    ...
    def start_tcp_bench(self, host="127.0.0.1", port=0, timeout=60,
        ...
        # The disposable bench disk receives the complete twelve-file package
...
TOOLS = {
    ...
    "msx_agent_listen": (t_agent_listen,
        "Run the MCP host as a TCP/IPv4 server and accept one reverse "
        "connection initiated by an MSX resident agent or transparent "
        "hardware adapter. Use this when the physical adapter is configured "
    ...
    "msx_file_put": (t_file_put,
        ...
        "advertises a decoder. The negotiated fast-v1 foreground helper PUT "
        "capacity is 2026 sequential data bytes per frame on 8251/UNAPI and "
        "106 on 16C550, with no extra STATUS round trip per block. Whole-file "
        "CRC, sparse durable state, "
    ...
    "msx_file_get": (t_file_get,
        ...
        "The negotiated fast-v1 foreground helper GET capacity is 2040 data "
        "bytes per frame on 8251/UNAPI and 120 on 16C550, replacing per-block "
        "GET ACKs with sparse durable checkpoints. "
